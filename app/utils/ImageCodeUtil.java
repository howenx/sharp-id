...
public class ImageCodeUtil {
	...
	private static volatile ImageCodeUtil imageCodeService = null;
	public static ImageCodeUtil getInstance() {
		ImageCodeUtil result = imageCodeService;
        if (result == null) {
            synchronized (ImageCodeUtil.class) {
                result = imageCodeService;
                if(result == null) {
                	imageCodeService = result =  new ImageCodeUtil();
                }
            }
        }
        return result;
  ...
  //使用到Algerian字体，系统里没有的话需要安装字体，字体只显示大写，去掉了1,0,i,o几个容易混淆的字符  
    ...
    public InputStream getImageIo(String code) {
        BufferedImage bi = new ImageCodeUtil().generate(120, 40, code);
