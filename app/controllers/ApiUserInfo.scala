...
class ApiUserInfo @Inject()(form: FormConstraint, cache_client: MemcachedClient, @Named("sms") sms: ActorRef, @Named("oss") oss: ActorRef, @Named("coupons") couponsActor: ActorRef, configuration: Configuration) extends Controller {
      ...
      data => {
        if (request.headers.get("id-token").isDefined) {
          ...
          if (user_id.isDefined) {
            ...
            val realYn = "Y"
            if (UserInfoModel.changeRealName(user_id.get.toLong, data.cardNum.get, json, data.realName.get, realYn) > 0) {
      ...
      if (request.headers.get("id-token").isDefined) {
        ...
        if (id_token != null && id_token != Nil) {
          ...
          if (user_id.isDefined) {
            ...
            //查询当前用户下所有未使用的优惠券数量
            val couponVo: List[CouponsVo] = CouponsModel.searchAllCoupons(CouponsVo(None, Some(user_id.get.toLong), None, None, None, None, Some("N"), None, None, None, None))
            UserInfoModel.findByUserId(user_id.get.toLong) match {
              case Some(user) =>
